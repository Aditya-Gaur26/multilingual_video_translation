...
This copy of Wav2Lip is vendored directly into the
`multilingual_video_translation` project.  The original repository is:
...
|------|------|--------|--------|
| `audio.py` | 100 | Changed `librosa.filters.mel(hp.sample_rate, hp.n_fft, ...)` to `librosa.filters.mel(sr=hp.sample_rate, n_fft=hp.n_fft, ...)` | `librosa >= 0.10` removed positional arguments from `filters.mel()`; all callers must now use keyword arguments or get a `TypeError`. |

### Model weights (not included in repo)

The following checkpoint files are excluded via `.gitignore` (combined ~500 MB):

| File | Size | Download |
|------|------|----------|
| `checkpoints/wav2lip_gan.pth` | ~416 MB | [Rudrabha's SharePoint](https://iiitaphyd-my.sharepoint.com/:u:/g/personal/radrabha_m_research_iiit_ac_in/EdjI7bZlgApMqsVoEUUXpLsBxqXbn5z8dvkNr3DTNPkCRg?e=CdW6FH) |
| `face_detection/detection/sfd/s3fd.pth` | ~86 MB | [adrianbulat.com](https://www.adrianbulat.com/downloads/python-fan/s3fd-619a316812.pth) |

Run `python setup_wav2lip.py` from the repo root to download both files automatically.
